...
router.get("/admin", requireLogin, async (req, res) => {
  try {
    ...
    // Global counts (including user count)
    const [pCountRes, eCountRes, sCountRes, mCountRes, dSumRes, uCountRes] = await Promise.all([
      ...
      db.query("SELECT COUNT(*) AS count FROM users")  // NEW: count all users
    ]);
    ...
    const userCount = Number(uCountRes.rows[0].count);  // NEW
    ...
    res.render("dashboard/admin", {
      ...
      milestoneTotal,
      donationTotal,
      userCount,  // NEW: pass userCount to the view
